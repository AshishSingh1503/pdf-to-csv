...
1. After a batch completes, run the SQL validation queries in `database_validation_queries.sql` (see the SQL file for exact queries).
...
---
For step-by-step SQL queries and automated scripts, see `database_validation_queries.sql` and `test_scripts/batch_upload_test.js`.
